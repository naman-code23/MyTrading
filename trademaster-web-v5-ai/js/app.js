...
function renderCoach() {
  ...
  const report = buildAiCoachReport(state.trades, state.settings.pnlMethod || PNL_METHODS.AVERAGE);
  ...
  refs.coachPatternCards.innerHTML = report.patternCards.length
    ? report.patternCards.map((item) => `
      ...
      `).join('')
    : '<div class="empty-state">Start tagging strategies and MBI snapshots to unlock more pattern cards.</div>';
}
...
function renderCharts() {
  ...
  if (state.ui.activeTab === 'coach') {
    const report = buildAiCoachReport(state.trades, method);
    chartManager.renderBar(
      'mbiBucket',
      $('#mbiBucketChart'),
...
function renderMbi() {
  const mbi = calculateMbi({
    ...
    nhl: $('#mbiNhl').value,
    bd: $('#mbiBd').value,
    prevScore: $('#mbiPrevScore')?.value,
    entryScore: $('#mbiEntryScore')?.value,
    daysHeld: $('#mbiDaysHeld')?.value,
    currentPnl: $('#mbiCurrentPnl')?.value,
  });
  ...
  if (!mbi.ready) {
    ...
    $('#mbiStatsText').textContent = 'Backtested regime stats will appear here.';
    $('#mbiSignals').innerHTML = '<div class="panel-note">Overrides will populate once all breadth inputs are filled.</div>';
    $('#mbiSellSignals').innerHTML = '<div class="panel-note">Sell-context signals appear when you add position context.</div>';
    $('#mbiBreakdown').innerHTML = '';
    $('#mbiSizing').innerHTML = '';
    return;
  }

  const scoreColor = mbi.color === 'green' ? 'var(--green)' : mbi.color === 'amber' ? 'var(--amber)' : mbi.color === 'red' ? 'var(--red)' : 'var(--slate)';
  $('#mbiScore').textContent = String(mbi.score);
  $('#mbiScore').style.color = scoreColor;
  $('#mbiZone').textContent = `${mbi.zone} • bucket ${mbi.bucket}`;
  $('#mbiZone').style.color = scoreColor;
  $('#mbiActionText').innerHTML = `<strong>Swing:</strong> ${escapeHtml(mbi.swingAction)} <br /><strong>Intraday:</strong> ${escapeHtml(mbi.intradayAction)}`;
  $('#mbiStatsText').textContent = mbi.stats;

  $('#mbiSignals').innerHTML = mbi.signals.length
    ? mbi.signals.map((signal) => `<div class="signal ${signal.level}"><strong>${escapeHtml(signal.label)}:</strong> ${escapeHtml(signal.text)}</div>`).join('')
    : '<div class="panel-note">No major overrides. Base zone rules apply.</div>';

  $('#mbiSellSignals').innerHTML = mbi.sellSignals.length
    ? mbi.sellSignals.map((signal) => `<div class="signal ${signal.level}"><strong>${escapeHtml(signal.label)}:</strong> ${escapeHtml(signal.text)}</div>`).join('')
    : '<div class="panel-note">No active sell signals.</div>';
  ...
  $('#mbiSizing').innerHTML = mbi.sizing.maxTradeValue
    ? [
        ...
        makeMetricPreviewCard('Intraday', mbi.sizing.intradayAllowed ? 'Allowed' : 'Skip', mbi.sizing.intradayAllowed ? 'positive' : 'warning'),
      ].join('')
...
function bindToolbarEvents() {
  ...
  refs.copyCoachPromptBtn?.addEventListener('click', async () => {
    const prompt = state.ui.lastCoachPrompt || buildAiCoachReport(state.trades, state.settings.pnlMethod || PNL_METHODS.AVERAGE).promptText;
    if (!prompt) {
      showToast('Add some closed trades first to generate a review prompt.', 'error');
      return;
    }
    try {
  ...
  [
    '#mbiA20', '#mbiA50', '#mbiA200', '#mbiNb', '#mbiWh', '#mbiWl', '#mbiBosf', '#mbiUhlh', '#mbiVol', '#mbiAdv', '#mbiNhl', '#mbiBd', '#mbiPrevScore', '#mbiEntryScore', '#mbiDaysHeld', '#mbiCurrentPnl',
