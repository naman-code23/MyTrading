...
| “We treated screenshots as a file lifecycle, rather than putting image bytes into journal documents.” | Browser image preparation, Storage upload, Firestore references, and old-image cleanup are implemented. The form retains input on save failures. Cloud upload and recovery paths still need testing. |
...
| “We created a repeatable rehearsal instead of relying on personal trading records.” | The verification report records 12 passing tests and a synthetic fixture with 12 trades and four winner examples, plus desktop and mobile browser checks. Synthetic results are demonstration data, not evidence of trading performance. |
...
| Documentation drifted as scope changed | “Earlier plans included optional branches that were no longer part of the web demo.” | Label every capability as implemented, configured, verified or proposed. In particular, web SMS is not PNV; optional App Check code is not confirmed enforcement; reserved verification paths are not a deployed Functions backend. |
...
| Cloud Functions | No present requirement | Client SDKs plus rules can serve this personal CRUD application. Consider Functions when there is a concrete need for privileged validation, secrets, scheduled cleanup, derived data or trusted external integrations. It would not by itself provide an atomic Firestore-plus-Storage commit. |
...
`firebase.json` publishes `.` and only ignores the config file, hidden files and node_modules. Tests, fixtures, package metadata and rule files in that directory can therefore enter the Hosting artifact. Use a dedicated public/build directory or deliberate excludes, with a deploy-file inspection. This is deploy hygiene; this review did not establish that secrets were exposed. See [Hosting configuration](https://firebase.google.com/docs/hosting/full-config).
...
| Unit/UI checks | The [verification report](/Users/naman/Documents/Coding/MyTrading/docs/SHOWCASE-VERIFICATION.md) records 12 passing tests and a synthetic desktop/mobile rehearsal. They were not rerun for this read-only assessment. |
