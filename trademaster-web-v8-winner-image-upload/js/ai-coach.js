...
function buildNarrative({ summary, bestStrategy, worstStrategy, bestMbi, weakMbi, bestHold, weakHold, bestWeekday, worstWeekday }) {
  ...
  if (bestMbi && weakMbi && bestMbi.label !== weakMbi.label) {
    parts.push(`Your best regime is ${bestMbi.label} SuperMBI, while ${weakMbi.label} is the weakest regime in the log.`);
  }
...
export function buildAiCoachReport(trades, method) {
  ...
  if (!closed.length) {
    return {
      ...
      strengths: [],
      leaks: [],
      actions: ['Import your broker tradebook CSV to seed the journal faster.', 'Tag each trade with a strategy and SuperMBI snapshot for sharper analysis.'],
      patternCards: [],
      summary,
      series: {
        mbiBuckets: [],
  ...
  if (bestMbi && bestMbi.pnl > 0) addUnique(strengths, `${bestMbi.label} SuperMBI trades are the strongest market regime in your journal.`);
  ...
  if (weakMbi && weakMbi.pnl < 0) addUnique(leaks, `${weakMbi.label} SuperMBI trades are underperforming and should be filtered harder.`);
  ...
  if (weakMbi && weakMbi.pnl < 0) addUnique(actions, `Add a hard filter: avoid fresh trades in ${weakMbi.label} SuperMBI conditions unless the setup is exceptional.`);
  ...
  if (missingMbiCount > 0) addUnique(actions, `Capture the SuperMBI snapshot on every trade. ${missingMbiCount} closed trades are missing regime data.`);
  ...
  if (!actions.length) addUnique(actions, 'Keep importing trades and tagging strategy + SuperMBI so the coach can generate sharper improvement plans.');
  ...
  const patternCards = [
    ...
    bestMbi ? { label: 'Best SuperMBI bucket', value: bestMbi.label, note: `₹${currencyNumber(bestMbi.pnl)} • ${bestMbi.winRate}% WR`, tone: 'positive' } : null,
    weakMbi ? { label: 'Weak SuperMBI bucket', value: weakMbi.label, note: `₹${currencyNumber(weakMbi.pnl)} • ${weakMbi.winRate}% WR`, tone: weakMbi.pnl < 0 ? 'negative' : 'warning' } : null,
