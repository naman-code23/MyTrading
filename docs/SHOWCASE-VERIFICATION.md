...
- `npm test` — 20 tests passed, including the Functions cleanup contracts.
- `node --check` passed for all `js/*.js`, `fixtures/*.mjs`, `functions/*.js`, and `tests/*.mjs`.
- `git diff --check` passed.
- `npx firebase-tools@latest functions:list --project trading-d5a0e --json` confirmed both screenshot-cleanup Functions are `ACTIVE` in `us-central1`, with Firestore triggers in `nam5`.
...
- Fixture manifest test confirms 12 trades across three months, 10 closed, 6 wins, 3 losses, 1 breakeven, 2 open, 4 winner examples, 2 linked winners, and 60% win rate. Fixed weighted-average totals are calculated into the manifest: net P&L `₹6,235`, gross profit `₹8,258`, gross loss `₹2,023`; cumulative and monthly chart totals use the same method.
...
These are deliberate environment/deployment boundaries, not claims that the live Firebase project is healthy. Hosting, rules, and live data rehearsals still require a separate release review and explicit authorization.
